...
class KumbiaActiveRecord {
    ...
    public function count($what = '') {
        ...
        if (isset($what['distinct']) && $what['distinct']) {
            ...
            if (isset($what['group'])) {
                $select = "SELECT COUNT(*) FROM (SELECT {$what['columns']} FROM $table ";
                $select.= $this->convert_params_to_sql($what);
                $select.= ') AS t ';
            } else {
            ...
            foreach ($this->_validates['presence_of'] as $f => $opt) {
                if (isset($this->$f) && (is_null($this->$f) || $this->$f == '')) {
                    ...
                    if (isset($opt['message'])) {
                        throw new NegocioExcepcion($opt['message']);
//                        Flash::error($opt['message']);
//                        return false;
                    } else {
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("Error: El campo $field no puede ser nulo");
...
//                        return false;
        ...
        foreach ($this->not_null as $f) {
            ...
            if (!isset($this->$f) || is_null($this->$f) || $this->$f == '') {
                ...
                throw new NegocioExcepcion("Error: El campo $f no puede ser nulo");
//                Flash::error();
                return false;
            }
        }
                ...
                if (isset($this->$f) && !is_null($this->$f) && $this->$f != '') {
                    ...
                    if (strlen($this->$f) < $opt['min']) {
                        if (isset($opt['too_short']))
                            throw new NegocioExcepcion($opt['too_short']);
//                            Flash::error($opt['too_short']);
                        else
                            throw new NegocioExcepcion("Error: El campo $field debe tener como mínimo $opt[min] caracteres");
...
//                        return false;
                    ...
                    if (strlen($this->$f) > $opt['max']) {
                        if (isset($opt['too_long']))
                            throw new NegocioExcepcion($opt['too_long']);
//                            Flash::error($opt['too_long']);
                        else
                            throw new NegocioExcepcion("Error: El campo $field debe tener como máximo $opt[max] caracteres");
...
//                        Flash::error($opt['message']);
                        ...
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("$field debe tener un valor entre (" . join(",", $opt['list']) . ")");
...
//                        Flash::error($opt['message']);
                        ...
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("$field no debe tener un valor entre (" . join(",", $opt['list']) . ")");
...
//                        Flash::error($opt['message']);
                        ...
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("$field debe tener un valor numérico");
...
//                        Flash::error($opt['message']);
                        ...
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("Formato erroneo para $field");
...
//                        Flash::error($opt['message']);
                        ...
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("Formato de fecha erroneo para $field");
...
//                        Flash::error($opt['message']);
                        ...
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("Formato de e-mail erroneo en el campo $field");
...
//                        Flash::error($opt['message']);
                        ...
                        $field = isset($opt['field']) ? $opt['field'] : $f;
                        throw new NegocioExcepcion("El valor '{$this->$f}' ya existe para el campo $field");
...
//            Flash::error('No se puede actualizar porque el registro no existe');
            throw new NegocioExcepcion('No se puede actualizar porque el registro no existe');
//            return false;
        }
    }
    ...
    public function secuencia($nombre){
        $seq = $this->db->fetch_one("SELECT nextval('$nombre') as secuencia");
        return $seq[0];
    }
