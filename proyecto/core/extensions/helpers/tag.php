...
class Tag
{
     ...
     */
    public static function js($src, $cache = TRUE,$path="")
    {
        if($path){
            $path = $path."/";
        }
     ...
     * @param string $media medio de la hoja de estilo
     */
    public static function echo_css($src, $media = 'screen',$path="")
    {
        if($path){
            $path = "$path/";
        }
        return '<link href="' . PUBLIC_PATH . "{$path}css/$src.css\" rel=\"stylesheet\" type=\"text/css\" media=\"$media\" />" . PHP_EOL;
    }
