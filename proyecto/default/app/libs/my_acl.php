...
class MyAcl {
     ...
     */
    public function check($modulo = NULL, $controlador = NULL, $accion = NULL) {
        $usuario = Auth::get('id');
        if ($modulo == NULL && $controlador == NULL && $accion == NULL) {
            $modulo = Router::get('module');
            $controlador = Router::get('controller');
            $accion = Router::get('action');
        }
        if (Auth::get('clave_blanqueada') && "$modulo/$controlador/$accion" == "admin/usuarios/ingreso_contrasenia") {
            MyAuth::cerrar_sesion();
            Session::set("usuario_blanqueado", $usuario);
            return true;
        }
        $urls_especiales ["///"]="///";
        $urls_especiales ["//index/index"]="/index/index";
        $urls_especiales ["/admin/index/logout"]="/admin/index/logout";
        if(array_key_exists("/$modulo/$controlador/$accion", $urls_especiales)){
            return true;
        }
